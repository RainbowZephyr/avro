...
module Avro
  module IO
    ...
    class BinaryDecoder
      ...
      def read_float : Float32
        ...
        read_and_unpack(4, 'e').as(Float32)
      end
      ...
      def read_double : Float64
        ...
        # little-endian format.
        read_and_unpack(8, 'E').as(Float64)
      end
      ...
      def read_string : String
        ...
        read_bytes.to_s(Encoding::UTF_8)
      end
      ...
      private def read_and_unpack(byte_count, format)
    ...
    class BinaryEncoder
      ...
      def write_boolean(datum : Bool)
        on_disk = datum ? 1_u8 : 0_u8
        writer.write_byte(on_disk)
      end

      # int and long values are written using variable-length,
      # zig-zag coding.
      def write_int(n : Int32)
        write_long(n)
      end

      # int and long values are written using variable-length,
      # zig-zag coding.
      def write_long(n : Int64)
        n = (n << 1) ^ (n >> 63)
        while (n & ~0x7F) != 0
          @writer.write_byte(((n & 0x7f) | 0x80).to_u8)
          n >>= 7
        end
        @writer.write_byte(n.to_u8)
      end
      ...
      # equivalent to Java's floatToIntBits and then encoded in
      # little-endian format.
      def write_float(datum : Float32)
        @writer.write(datum.to_a(Float32).pack('e'))
      end
      ...
      # equivalent to Java's doubleToLongBits and then encoded in
      # little-endian format.
      def write_double(datum : Float64)
        @writer.write(datum.to_a(Float64).pack('E'))
      end

      # Bytes are encoded as a long followed by that many bytes of data.
      def write_bytes(datum : Bytes)
      ...
      # A string is encoded as a long followed by that many bytes of
      # UTF-8 encoded character data
      def write_string(datum : String)
        write_bytes(datum.to_slice)
      end
    ...
    class DatumReader
      ...
      property writers_schema : Schema | Nil
      property readers_schema : Schema | Nil

      def initialize(@writers_schema : Schema? = nil, @readers_schema : Schema? = nil)
      end

      def read(decoder : BinaryDecoder)
        @readers_schema = writers_schema unless readers_schema
        read_data(writers_schema!, readers_schema!, decoder)
      end

      def read_data(writers_schema : Schema, readers_schema : Schema, decoder : BinaryDecoder)
        # schema matching
      ...
      def read_fixed(writers_schema : Schema, _readers_schema : Schema, decoder : BinaryDecoder)
        decoder.read(writers_schema.size)
      end

      def read_enum(writers_schema : Schema, readers_schema : Schema, decoder : BinaryDecoder)
      ...
      def read_array(writers_schema : Schema, readers_schema : Schema, decoder : BinaryDecoder)
        read_items = [] of String
      ...
      def read_map(writers_schema : Schema, readers_schema : Schema, decoder : BinaryDecoder)
        read_items = {} of String => String
        block_count = decoder.read_long
        while block_count != 0
          if block_count < 0
            block_count = -block_count
      ...
      def read_union(writers_schema : Schema, readers_schema : Schema, decoder : BinaryDecoder)
      ...
      def read_record(writers_schema : Schema, readers_schema : Schema, decoder : BinaryDecoder)
      ...
      def read_default_value(field_schema : Schema, default_value)
      ...
      def skip_data(writers_schema : Schema, decoder : BinaryDecoder)
 ...
 :long
      ...
      def skip_fixed(writers_schema : Schema, decoder : BinaryDecoder)
        decoder.skip(writers_schema.size)
      end

      def skip_enum(_writers_schema : Schema, decoder : BinaryDecoder)
        decoder.skip_int
      end

      def skip_union(writers_schema : Schema, decoder : BinaryDecoder)
        index = decoder.read_long
        skip_data(writers_schema.schemas[index], decoder)
      end

      def skip_array(writers_schema : Schema, decoder : BinaryDecoder)
        skip_blocks(decoder) { skip_data(writers_schema.items!, decoder) }
      end

      def skip_map(writers_schema : Schema, decoder : BinaryDecoder)
        skip_blocks(decoder) {
          decoder.skip_string
      ...
      def skip_record(writers_schema : Schema, decoder : BinaryDecoder)
        writers_schema.fields.each{|f| skip_data(f.type, decoder) }
      end

      private def skip_blocks(decoder : BinaryDecoder, &blk)
        block_count = decoder.read_long
        while block_count != 0
          if block_count < 0
    ...
    class DatumWriter
      ...
      property writers_schema : Schema | Nil
      def initialize(@writers_schema : Schema? = nil)
      end

      def write(datum, encoder : BinaryEncoder)
        write_data(writers_schema!, datum, encoder)
      end

      def write_data(writers_schema : Schema, logical_datum, encoder : BinaryEncoder)
      ...
      def write_fixed(_writers_schema : Schema, datum, encoder : BinaryEncoder)
        encoder.write(datum)
      end

      def write_enum(writers_schema : Schema, datum, encoder : BinaryEncoder)
        index_of_datum = writers_schema.symbols.index(datum)
        encoder.write_int(index_of_datum)
      end

      def write_array(writers_schema : Schema, datum, encoder : BinaryEncoder)
      ...
      def write_map(writers_schema : Schema, datum, encoder : BinaryEncoder)
      ...
      def write_union(writers_schema : Schema, datum, encoder : BinaryEncoder)
      ...
      def write_record(writers_schema : Schema, datum, encoder : BinaryEncoder)
        raise AvroTypeError.new(writers_schema, datum) unless datum.is_a?(Hash)
        writers_schema.fields.each do |field|
